...
update book set publication_time='2018-10-1' where author='老舍';
4. 修改所有中国文学出版社出版的作为 出版时间为 2020-1-1
update book set publication_time='2020-1-1' where press='中国文学出版社';
5. 修改所有出版时间为Null的图书 出版时间为 2019-10-1
update book set publication_time='2019-10-1' where publication_time is null;
6. 所有鲁迅的图书价格增加5元
...
* 限制
  ...
  OFFSET为跳过的意思，如：limit 1 offset 2;跳过2个后取第一个。
...
* 子查询
	...
	* 子查询出现的位置：
		1. from 之后 ，此时子查询的内容作为一个新的表内容，再进行外层select查询（以下两种方式均可）；
		```sql
		select name from (select * from class_1 where sex='m') as s where s.score > 90;
		select name from (select * from class_1 where sex='m') as s where score > 90;
		```
	...
	2. where字句中，此时select查询到的内容作为外层查询的条件值
		...
		```sql
		 	...
		 	select * from cls where score >(select score from cls where name='Lily');
		 	-- 查找分数大于Lily的所有人
		 	select * from cls where name in (select name from hobby);
		 	-- 查找cls表中同时也在hobby表中存在的名字（即查询为报了兴趣班的同学信息）
		```
		> 注意：
		>
...
8. 找到魏国排名2-3名的英雄
select * from sanguo where country='魏' order by attack desc,defense desc limit 2 offset 1;
...
| min(字段名)   | 该字段的最小值                                           |
| sum(字段名)   | 该字段所有记录的和                                       |
| count(字段名) | 统计该字段记录的个数(不会统计NULL值，一般统计id比较靠谱) |
|               |                                                          |
...
limit 2;
select country,count(id) from sanguo where gender='男' group by country order by count(id) desc;	-- 第二种方法
```
...
limit 2;
-- 查询平均防御力大于50的最高的平均防御力的且为男性的国家和平均防御力信息，并显示为国家名称和平均防御力
select country as 国家,avg(defense) as 平均防御 from sanguo where gender='男' group by country having avg(defense) > 50 order by avg(defense) desc limit 1;
...
select press,avg(price) from book where price > 50 group by press order by avg(price) desc; 
-- 上面为误导，下面为正确答案
select press,avg(price) from book where press in (select press from book where price > 50) group by press order by avg(price) desc;
-- 方法2；
select press,avg(price) from book group by press having max(price) > 50 order by avg(price) desc;
5. 统计同一时间出版图书的最高价格和最低价格
select publication_time,max(price),min(price) from book group by publication_time;
```
...
- foreign key 外键的定义语法：
  ...
  ```mysql
  [CONSTRAINT symbol](给外键取一个名字，可写可不写，外键名字不能重复) FOREIGN KEY（外键字段,即从表） 
...
3. 查询各科成绩最高和最低的分数,形式 : 课程ID	 课程名字  最高分  最低分
select cid,cname,max(number),min(number) from
course left join score on course.cid=score.course_id
group by cid,cname;
...
group by student.sid having avg(number)>85;
...
1.将book拆分为三个数据表 分别存储：图书信息(book_information)	作家信息	出版社信息
...
create table author_information(
    ...
    aname varchar(30),
    book_id int unique,
    foreign key (book_id) references book_information(bid));
...
create table press_information(
    ...
    book_bid int,
    author_aid int,
    foreign key (book_bid) references book_information(bid),
    foreign key (author_aid) references author_information(aid));
```
...
* 视图表的增删改查操作(如视图是针对多张表创建的，则很少做写操作，基本都做读操作)
...
1.编写一个函数，传入两个同学的ID值，返回两位同学的分数之差

```mysql
delimiter $$
create function poor_grades(sid1 int,sid2 int) returns int
begin
declare a int;
declare b int;
select score from cls where id=sid1 into a;
select score from cls where id=sid2 into b;
return a-b;
end $$
delimiter ;
```
...
2.编写一个存储过程，通过用户变量传入学生的姓名，使用该用户变量得到这个学生的成绩

```mysql
delimiter $$
create procedure get_score2(inout stu varchar(30))
begin
...
* **如何选择存储引擎**
	...
	```mysql
	1. 执行查操作多的表用 MyISAM(使用InnoDB浪费资源)（使用查询操作优先）
	2. 执行写操作多的表用 InnoDB（使用写操作优先，一般使用此引擎）
...
```mysql
create table good_score select * from cls where score>80;
+----+-------+------+-----+-------+
...
|  6 | Emma  |   16 | w   |    99 |
+----+-------+------+-----+-------+
create table bad_score select * from cls where score<80;
+----+---------+------+-----+-------+
| id | name    | age  | sex | score |
+----+---------+------+-----+-------+
|  3 | Tom     |   18 | m   |    68 |
...
>
> ```mysql
> > tarena@tedu:~/桌面/xuwenbin/month02/day09$ mysqldump -u root -p stu > stu.sql
> ```
>
> 恢复命令格式
...
>
> ```mysql
> > tarena@tedu:~/桌面/xuwenbin/month02/day09$ mysql -u root -p student < stu.sql
> ```
>
> 
