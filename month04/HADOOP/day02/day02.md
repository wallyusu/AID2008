...
4. 将本地students.txt文件上传到HDFS中
   ...
   `hadoop fs -put student.txt /studir/stuinfo/`

5. 查看HDFS中 /studir/stuinfo/student.txt 的内容

   `hadoop fs -text /studir/stuinfo/student.txt`

6. 将HDFS中 /studir/stuinfo/students.txt 下载到本地命名为 new_students.txt

   `hadoop fs -get /studir/stuinfo/student.txt /home/tarena/桌面/new_students.txt`
...
- **Hive优点**
  ...
  3. 模型简单，易于理解
  4. 针对海量数据(数据仓库)的高性能查询和分析
  5. 与 Hadoop 的其他产品完全兼容
- **Hive缺点**
  1. 不支持行级别的增删改
  2. 不支持完整的在线事务处理
...
load data local inpath '/home/tarena/data/product.txt' into table product_tab;
```
